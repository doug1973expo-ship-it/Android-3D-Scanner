...
class _HomeScreenState extends State<HomeScreen> {
      ...
      appBar: AppBar(
        title: const Row(
          children: [
            ...
            SizedBox(width: 10),
            Text('RoomScope', style: TextStyle(fontWeight: FontWeight.w700)),
          ],
        ),
        actions: [
          Padding(
      ...
      children: [
              ...
              children: [
                ...
                if (showMap)
                  ColoredBox(
                    ...
                    child: CloudViewer(points: controller.map.points.toList()),
                  ),
                if (!controller.ready)
                  ColoredBox(
                ...
                Positioned(
                  top: 14,
                  left: 14,
                  child: _Pill(showMap ? '3D POINT CLOUD' : 'LIVE CAMERA'),
                ),
                Positioned(
                  top: 8,
                  right: 8,
        ...
        FilledButton.icon(
          style: FilledButton.styleFrom(
            backgroundColor: controller.recording ? Colors.redAccent : mint,
                        ...
                        Expanded(
                          child: InkWell(
                            onTap: () => Navigator.of(context).push<void>(
                              MaterialPageRoute(
                                builder: (_) => SavedRoomScreen(capture: item),
                              ),
                            ),
                            c
                ),
  ...
  Widget _cloudPage() => ListView(
   ...
   ren: [
      ...
      const SizedBox(height: 16),
      const Text(
        'Room mapping happens on your device. '
        'Only captures you choose are uploaded to your account.',
        textAlign: TextAlign.center,
      ...
      if (!controller.cloud.configured)
        const Padding(
          ...
          child: Text(
            'Cloud sync is not available in this build. You can still save captures on this device.',
            textAlign: TextAlign.center,
            style: TextStyle(color: Colors.white54),
          ),
        ),
                  ...
                  return const Center(
                    child: Padding(
                      ...
                      child: Text(
                        ...
                        'Its saved files are still on this device.',
                        textAlign: TextAlign.center,
                      ),
                    ),
                  );
...
class _CloudViewerState extends State<CloudViewer> {
  double yaw = .25, pitch = -.2, zoom = 70, _startZoom = 70;
  @override
...
class _CloudPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < points.length; i += step) {
      final p = points[i];
      final x = (p.x - cx) * math.cos(yaw) + (p.z - cz) * math.sin(yaw);
      final z = -(p.x - cx) * math.sin(yaw) + (p.z - cz) * math.cos(yaw);
